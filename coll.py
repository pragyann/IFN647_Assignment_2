...
def parse_documents(inputpath, stop_words):
    """Parse an RCV1 data files into a collection.

    inputpath is the folder name of the RCV1 data files. The parsed collection
    is returned as BowColl.  NOTE the function performs very limited error checking."""
...
def parseQuery(query, stop_words):
    """
    Function that parses query.\n
    Parameters: `query` - query string to be parsed, `stop_words` - list of stop words to exclude during parsing\n
    Return value: parsed query as dictionary `{term: frequency}`
    """
    # Getting rid of numbers and punctuations
    cleaned_query = query.translate(str.maketrans("", "", string.digits)).translate(
    ...
    for word in words:
        ...
        # Length check and stop word removal
        if len(term) > 2 and term not in stop_words:
            terms.append(term)
